...
class Table:
    ...
    def __init__(self, headers=None, rows=None, style='light', alignments=None, 
                 widths=None, padding=1, title=None, border_color=None):
        """
        Initialize a table.
    ...
    def _calculate_widths(self):
        ...
        num_cols = max(
            len(self.headers),
            max([len(row) for row in self.rows]) if self.rows else 0
        )
