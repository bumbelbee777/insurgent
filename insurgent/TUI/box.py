...
class Box:
    ...
    def __init__(self, width=None, style='single', title=None, color=None, 
                 padding=1, align='left', margin=0):
        """
    ...
    def wrap_content(self, content, width=None, height=None):
        ...
        if width is None:
            ...
            content_width = max([Text.visible_length(line) for line in content]) if content else 0
            # Add padding
            ...
            if self.title:
                title_width = Text.visible_length(self.title) + 4  # +4 for borders and spacing
        ...
        if self.title:
            ...
            if remaining_width >= 0:
                ...
                truncated_title = Text.truncate(self.title, width - 6)  # -6 for corners and spaces
                top_border += f" {truncated_title} "
                top_border += h_char * (width - 4 - Text.visible_length(truncated_title))
        else:
    ...
    def top_border(self, width, with_title=True):
        ...
        if not with_title or not self.title:
            return (self.get_styled_char('top_left') + 
    ...
    def bottom_border(self, width):
        ...
        """
        return (self.get_styled_char('bottom_left') + 
                self.get_styled_char('horizontal') * (width - 2) + 
                self.get_styled_char('bottom_right'))
